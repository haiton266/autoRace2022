...
public:
    ...
    boolean run(); // CÓ XÀI GIA TỐC

    /// Poll the motor and step it if a step is due, implementing a constant
    /// speed as set by the most recent call to setSpeed(). You must call this as
    /// frequently as possible, but at least once per step interval,
    /// \return true if the motor was stepped.
    boolean runSpeed(); // TỐC ĐỘ CỐ ĐỊNH
    ...
    void setMaxSpeed(float speed); // GIA TỐC CHO ĐỦ TỐC ĐỘ MAX THÌ DỪNG

    /// Returns the maximum speed configured for this stepper
    /// that was previously set by setMaxSpeed();
    /// \return The currently configured maximum speed
    float maxSpeed(); // TRẢ VỀ MAX SPEED HIỆN TẠI

    /// Sets the acceleration/deceleration rate.
    /// \param[in] acceleration The desired acceleration in steps per second
    /// per second. Must be > 0.0. This is an expensive call since it requires a square
    /// root to be calculated. Dont call more ofthen than needed
    void setAcceleration(float acceleration); // SET GIA TỐC
    ...
    void setSpeed(float speed); // SET ĐỂ CHẠY VỚI TỐC ĐỘ CỐ ĐỊNH

    /// The most recently set speed.
    /// \return the most recent speed in steps per second
    float speed(); // TỐC ĐỘ GẦN ĐÂY NHẤT

    /// The distance from the current position to the target position.
    /// \return the distance from the current position to the target position
    /// in steps. Positive is clockwise from the current position.
    long distanceToGo(); // TRẢ VỀ SỐ BƯỚC CẦN PHẢI ĐI CÒN LẠI

    /// The most recently set target position.
    /// \return the target position
    /// in steps. Positive is clockwise from the 0 position.
    long targetPosition(); // TRẢ VỀ SỐ BƯỚC KHI ĐẠT ĐƯỢC Ở VỊ TRÍ MỤC TIÊU

    /// The current motor position.
    /// \return the current motor position
    /// in steps. Positive is clockwise from the 0 position.
    long currentPosition(); // TRẢ VỀ SỐ BƯỚC Ở VỊ TRÍ HIỆN TẠI
    ...
    void setCurrentPosition(long position); // SET LẠI VỊ TRÍ

    /// Moves the motor (with acceleration/deceleration)
    /// to the target position and blocks until it is at
    /// position. Dont use this in event loops, since it blocks.
    void runToPosition(); // CHẠY TỚI VỊ TRÍ RỒI
    ...
    boolean runSpeedToPosition(); // VỊ TRÍ MỤC TIÊU CÓ THỂ THAY ĐỔI NHƯNG VẪN CHẠY ĐƯỢC NHƯNG KHÔNG XÀI ĐƯỢC GIA TỐC

    /// Moves the motor (with acceleration/deceleration)
    /// to the new target position and blocks until it is at
    /// position. Dont use this in event loops, since it blocks.
    /// \param[in] position The new target position.
    void runToNewPosition(long position); // CHẠY TỚI VỊ TRÍ MỤC TIÊU RỒI KHÓA, XÀI ĐƯỢC GIA TỐC

    /// Sets a new target position that causes the stepper
    /// to stop as quickly as possible, using the current speed and acceleration parameters.
    void stop(); // ĐẶT VỊ TRÍ MỚI ĐỂ DỪNG, NHƯNG NHANH NHẤT CÓ THỂ VỚI TỐC ĐỘ HIỆN TẠI VÀ GIA TỐC HIỆN TẠI
